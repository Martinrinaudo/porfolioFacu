...
- **Cloudinary — integrated for upload.** `app/admin/(protected)/upload/page.tsx` fetches categories server-side and renders `upload-form.tsx` (Client Component: drag/drop, multi-file batch up to `MAX_BATCH_SIZE`, per-file preview/title, category select). Submits to the `uploadPhotos` Server Action in `actions.ts`, which validates, uploads concurrently via `lib/cloudinary/upload.ts` (`uploadPhotoToCloudinary`, capped to 2500×2500 on ingest, `folder: 'portfolio'`), and batch-inserts into `photos`. Shared validation constants live in `lib/cloudinary/validation.ts` (no SDK import, safe for the client bundle). `next.config.ts` sets `experimental.serverActions.bodySizeLimit: '100mb'` for the batch uploads.
  - **Gotcha**: `cloudinary`'s `v2` API (`lib/v2/uploader.js`, what `import { v2 as cloudinary } from 'cloudinary'` gives you) wraps the base uploader via a `v1_adapters` shim that expects `upload_stream(options, callback)` — options first. The *unwrapped* v1 module (`lib/uploader.js`) takes `(callback, options)`, which is what a lot of blog posts/older examples show and what the raw JS source looks like if you go grep it — easy to get backwards. Get the order wrong and it fails silently at runtime (hangs, then throws deep inside `utils/index.js`'s `v1_result_adapter`), not at the TypeScript level, since both are structurally callable. Trust the shipped `.d.ts` (`options` first) over the raw `lib/uploader.js` source.
- No public-facing page reads from Supabase yet — `app/page.tsx` is still the default `create-next-app` scaffold. Category management (creating/editing categories) also has no UI yet — done via the Supabase SQL editor/dashboard for now.
